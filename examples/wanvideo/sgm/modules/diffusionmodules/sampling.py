...
class VideoDDIMSampler(BaseDiffusionSampler):
    ...
    def denoise(self, x, input_data, denoiser, alpha_cumprod_sqrt, cond, uc, timestep=None, idx=None, scale=None, scale_emb=None):
        ...
        if 'pred_motion_frames' in input_data.keys():
            additional_model_inputs['pred_motion_frames'] = input_data['pred_motion_frames']
        try:
            additional_model_inputs['motion_len'] = input_data['motion_len']
            additional_model_inputs['motion_tokens_num'] = input_data['motion_tokens_num']
        except:
            print(f"WARNING.... motion_len and motion_tokens_num should be passed in denoise....")
        ...
        for key, value in input_data.items():
            if torch.is_tensor(value):
                ...
                # if key == "motion_frame":
                # # if key == "ref_image" or key == "motion_frame":
                #     print('drop:', key)
                #     additional_model_inputs[key] = torch.cat([value, value * 0])
                # else:
                #     additional_model_inputs[key] = torch.cat([value, value])
    ...
    def __call__(self, denoiser, x, input_data, cond, uc=None, num_steps=None, scale=None, scale_emb=None):
