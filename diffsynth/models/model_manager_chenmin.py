...
def load_model_from_single_file(state_dict, model_names, model_classes, model_resource, torch_dtype, device):
    ...
    for model_name, model_class in zip(model_names, model_classes):
        ...
        model_ref_state_dict = {}
        for k, v in model_state_dict.items():
            if 'transformer_blocks_copy' in k and 'self_attn' in k and '.k.' in k:
                k_new = k.replace('.k.', '.k_id.')
                if k_new not in model_state_dict.keys():
                    tmp = torch.zeros_like(v)
                    torch.nn.init.uniform_(tmp)
                    model_ref_state_dict[k_new] = tmp

                k_new = k.replace('.k.', '.k_rf.')
                if k_new not in model_state_dict.keys():
                    tmp = torch.zeros_like(v)
                    torch.nn.init.uniform_(tmp)
                    model_ref_state_dict[k_new] = tmp
                    
            elif 'blocks' in k and 'self_attn' in k and '.v.' in k:
                k_new = k.replace('.v.', '.v_id.')
                if k_new not in model_state_dict.keys():
                    tmp = torch.zeros_like(v)
                    torch.nn.init.uniform_(tmp)
                    model_ref_state_dict[k_new] = tmp
                k_new = k.replace('.v.', '.v_rf.')
                if k_new not in model_state_dict.keys():
                    tmp = torch.zeros_like(v)
                    torch.nn.init.uniform_(tmp)
                    model_ref_state_dict[k_new] = tmp
            elif 'blocks' in k and 'self_attn' in k and '.norm_k.' in k:
                k_new = k.replace('.norm_k.', '.norm_k_id.')
                if k_new not in model_state_dict.keys():
                    tmp = torch.zeros_like(v)
                    torch.nn.init.uniform_(tmp)
                    model_ref_state_dict[k_new] = tmp
                k_new = k.replace('.norm_k.', '.norm_k_rf.')
                if k_new not in model_state_dict.keys():
                    tmp = torch.zeros_like(v)
                    torch.nn.init.uniform_(tmp)
                    model_ref_state_dict[k_new] = tmp
        ...
        model.load_state_dict(model_ref_state_dict, strict=False, assign=True)

        print("Adding:" , model_ref_state_dict.keys())
