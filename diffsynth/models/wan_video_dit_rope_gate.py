...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        dtype = x.dtype
        return self.norm(x.float()).to(dtype) * self.weight.to(x.device)
...
class AttentionModule(nn.Module):
    ...
    def set_gate_train(self, train: bool = True):
...
class SelfAttention(nn.Module):
    def __init__(self, dim: int, num_heads: int, eps: float = 1e-6):
        ...
        self.attn = AttentionModule(self.num_heads,dim=dim)
...
class WanModel(torch.nn.Module):
    ...
    def forward(self,
        ...
        # x_ref, (f_ref, h_ref, w_ref) = self.patchify(x_ref)
        # spanned_f = torch.linspace(0, f, self.num_heads+2, dtype=torch.long)[1:-1]
        ...
        # freqs_ref_h = self.freqs[1][h : h + h_ref].view(1, h_ref, 1, -1).expand(self.num_heads, h_ref, w_ref, -1)
        # freqs_ref_w = self.freqs[2][w : w + w_ref].view(1, 1, w_ref, -1).expand(self.num_heads, h_ref, w_ref, -1)
