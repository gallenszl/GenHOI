...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if "vace_reference_image" in kwargs.keys():
        vace_reference_image = kwargs["vace_reference_image"]
        if vace_reference_image is not None and isinstance(vace_reference_image, list):
            f_ref = len(vace_reference_image)
        else:
            f_ref = 1
            
        h_ref, w_ref = h, w

        for i in range(f_ref):
            # print("f_ref:",i)
            spanned_f = torch.linspace(0, f, dit.num_heads+2, dtype=torch.long)[1:-1]
            freqs_ref_f = dit.freqs[0][spanned_f]
            freqs_ref_f = repeat(freqs_ref_f, 'nHead d -> nHead h_ref w_ref d',
                nHead=dit.num_heads, h_ref=h_ref, w_ref=w_ref)
            
            freqs_ref_h = dit.freqs[1][h + h_ref*i : h + h_ref*(i+1)].view(1, h_ref, 1, -1).expand(dit.num_heads, h_ref, w_ref, -1)
            freqs_ref_w = dit.freqs[2][w + w_ref*i : w + w_ref*(i+1)].view(1, 1, w_ref, -1).expand(dit.num_heads, h_ref, w_ref, -1)

            freqs_ref = torch.cat([freqs_ref_f, freqs_ref_h, freqs_ref_w], dim=-1)
            freqs_ref = rearrange(freqs_ref, 'nHead h_ref w_ref d -> (h_ref w_ref) nHead d').to(x.device)

            freqs_vace = freqs.expand(f * h * w, dit.num_heads, -1).clone()
            # import pdb; pdb.set_trace()
            freqs_vace[(freqs_ref.shape[0])*i:(freqs_ref.shape[0])*(i+1),:,:] = freqs_ref
    ...
    if vace_context is not None:
        vace_hints = vace(
            x, vace_context, context, t_mod, freqs_vace,
            use_gradient_checkpointing=use_gradient_checkpointing,
            use_gradient_checkpointing_offload=use_gradient_checkpointing_offload
        )
