    ...
    def __call__(
        ...
        if tracking_maps is not None:
            ...
            for i in range(gt_frames.shape[0]):
                pil_image = self.to_pil((pixel_values_ref_img[i, :, 0].float() + 1) / 2)
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            noise_pred_posi = self.dit(latents, tracking_maps=control_information, timestep=timestep, **prompt_emb_posi, **image_emb, **extra_input, x_ref = None, x_id = None)
            if cfg_scale != 1.0:
                noise_pred_nega = self.dit(latents, tracking_maps=control_information, timestep=timestep, **prompt_emb_nega, **image_emb, **extra_input, x_ref = None, x_id = None)
        ...
        return frames, frames_control, gt_frames, pixel_values_ref_img, pixel_values_ref_img_only_mask
