...
class WanVideoPipeline(BasePipeline):
    ...
    def fetch_models(self, model_manager: ModelManager):
        ...
        # self.dit = model_manager.fetch_model("wan_video_dit_rope_frameSpan")
    ...
    def __call__(
        ...
        if tracking_maps is not None:
            ...
            # import pdb; pdb.set_trace()
            latents_ref_img = torch.concat([pixel_values_ref_img, mask_input_for_v2v[:, :, 0:1]*0, pixel_values_ref_img], dim=1) #x, mask, conrol
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Inference
            noise_pred_posi = self.dit(latents, timestep=timestep, clip_feature=clip_feature, y=y, latents_ref_img=latents_ref_img, mask_input_for_v2v=mask_input_for_v2v, **prompt_emb_posi, **extra_input)
            if cfg_scale != 1.0:
                noise_pred_nega = self.dit(latents, timestep=timestep, clip_feature=clip_feature, y=y, latents_ref_img=latents_ref_img, mask_input_for_v2v=mask_input_for_v2v, **prompt_emb_nega, **extra_input)
